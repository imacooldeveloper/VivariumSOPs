...
final class SOPCategoryViewModel: ObservableObject {
    ...
    // Fecth the category List
    @MainActor
    func fecthCagetoryList() async throws {
        do {
//            categoryList = try await CategoryManager.shared.getAllCategory().filter{$0.categoryTitle == "Husbandry"}
            
            categoryList = try await CategoryManager.shared.getAllCategory().sorted {$0.categoryTitle < $1.categoryTitle}
//            for category in categoryList {
//                sopService?.nameOFCategory = category.categoryTitle
//
//                print(sopService?.nameOFCategory)
//            }
        } catch {
            print("erroring getting the catgeory from firebase")
        }
    }
